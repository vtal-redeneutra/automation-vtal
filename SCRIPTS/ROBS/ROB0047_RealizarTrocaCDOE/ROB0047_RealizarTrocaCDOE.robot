...
Trocar CDOE via SOM
    ...
    #VALIDA A ATIVIDADE T037 - Solicitar Troca de CDOE
    FOR    ${x}      IN RANGE     ${TENTATIVAS_FOR}
        ${EXIST}=  Run Keyword And Return Status    Wait for Elements State    (//*[normalize-space()="T037 - Solicitar Troca de CDOE"])[1]    Visible    10
            IF    ${EXIST} == True
            ...
            ELSE
                Click Web Element Is Visible            ${SOM_btn_refresh}
                IF    ${x} == ${TENTATIVAS_FOR}-1
                    Fatal Error             \nAtividade não foi encontrada no SOM
                END
            END
    END
    ...
    #TROCA CDOE
    ${numero_CDOE_Nova}=                    Set Variable                            2617
    ...
    Sleep                                   3s
    pause execution                         Clique no OK quando carregar a tela
