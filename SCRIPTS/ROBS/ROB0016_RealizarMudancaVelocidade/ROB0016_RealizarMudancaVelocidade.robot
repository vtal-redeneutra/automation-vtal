...
Mudar Velocidade
    ...
    IF    "${Complements_true_false}" != "false"
        ...
        ELSE
            ...
            ${Response}=                    POST_API                                ${API_BASEPRODUCTORDERING}/productOrder                               "order": {"correlationOrder": "${CORRELATIONORDER}","associatedDocument": "${ASSOCIATEDDOCUMENT}","associatedDocumentDate": "${ASSOCIATEDDOCUMENTDATE}","type": "${Type}","infraType": "${InfraType}","customer": {"name": "${NAME}","subscriberId": "${SUBSCRIBERID}","businessUnity": "${BUSINESSUNITY}","fantasyName": "${FANTASYNAME}","phoneNumber": {"phoneNumbers": ["${PHONENUMBER}","",""]}},"addresses": {"address": {"id": "${ADDRESSID}","inventoryId": "${INVENTORYID}","reference": "${Reference}","complement": {"complements": ${jsonComplements}}}},"products": {"product": [{"catalogId": "BL_${CatalogADD}MB","action": "adicionar"},{"catalogId": "BL_${CatalogREMOVE}MB","action": "remover"}]}}                   
            Set Global Variable             ${Response}
        END    
    

    ELSE
        ${Response}=                        POST_API                                ${API_BASEPRODUCTORDERING}/productOrder                               "order": {"correlationOrder": "${CORRELATIONORDER}","associatedDocument": "${ASSOCIATEDDOCUMENT}","associatedDocumentDate": "${ASSOCIATEDDOCUMENTDATE}","type": "${Type}","infraType": "${InfraType}","customer": {"name": "${NAME}","subscriberId": "${SUBSCRIBERID}","businessUnity": "${BUSINESSUNITY}","fantasyName": "${FANTASYNAME}","phoneNumber": {"phoneNumbers": ["${PHONENUMBER}","",""]}},"addresses": {"address": {"id": "${ADDRESSID}","inventoryId": "${INVENTORYID}","reference": "${Reference}"}},"products": {"product": [{"catalogId": "BL_${CatalogADD}MB","action": "adicionar"},{"catalogId": "BL_${CatalogREMOVE}MB","action": "remover"}]}}                   
        Set Global Variable                 ${Response}
    END
