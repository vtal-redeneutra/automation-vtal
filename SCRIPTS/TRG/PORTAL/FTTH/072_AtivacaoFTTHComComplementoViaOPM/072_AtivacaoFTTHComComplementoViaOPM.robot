...
*** Variables ***
${DAT_CENARIO}                              ${DIR_DAT}/072_AtivacaoFTTHComComplementoViaOPM.xlsx

*** Test Cases ***

72.01/02/03/04/05 - Criar Ordem de Instalação sem complementos via PORTAL
    [Tags]                                SCRIPT_A
...
# 72.06 - Validar a Notificação da Criação de Ordem no MicroServiços

72.07 - Validar a Criação da OS de Instalação via SOM
    [Tags]                                SCRIPT_B
...
72.XX - Atualização da data de agendamento via API
    [TAGS]                               SCRIPT_B
    Reagendar Pedido OPM e FSL

72.XX - Troca de técnico via FSL
    [TAGS]                                  SCRIPT_B
    Troca de Tecnico no Field Service
    Browser.Close Browser                           CURRENT

72.08 - Realiza o Encerramento da OS de Instalação
    [TAGS]                                  SCRIPT_B
...
72.09 - Validar mudanças de estado no FW 
    [TAGS]                                  SCRIPT_B
...
72.10 - Auditoria de Tarefas
    [TAGS]                                  SCRIPT_B
    Auditoria de Tarefas

72.11 - Realizar Validação de Retorno via SOM
    [TAGS]                                  SCRIPT_B
