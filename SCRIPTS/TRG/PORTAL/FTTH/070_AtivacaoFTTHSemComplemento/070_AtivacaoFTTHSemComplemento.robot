...
*** Variables ***
${DAT_CENARIO}                              ${DIR_DAT}/070_AtivacaoFTTHSemComplemento.xlsx

*** Test Cases ***

# 70.01/02/03/04/05 - Criar Ordem de Instalação sem complementos via PORTAL
#     [Tags]                                SCRIPT_A
...
70.06 - Validar a Notificação da Criação de Ordem no MicroServiços
    [Tags]                                SCRIPT_A
    
    Login ao Portal de Microserviços
    Acessar Microserviços no menu do PORTAL de Microserviços
    Procurar por ProductOrdering.listenerProductOrderCreateEvent referente ao casaId
    Extrair dado do Bloco                   START - Inicialização do serviço        id                                      addressId
    Extrair dado do Bloco                   START - Inicialização do serviço        associatedDocument                      associatedDocument
    Extrair dado do Bloco                   START - Inicialização do serviço        date                                    associatedDocumentDate

70.07 - Validar a Criação da OS de Instalação via SOM
    [Tags]                                SCRIPT_B
...
70.XX - Troca de Técnico
    [Tags]                                SCRIPT_B

    Troca de Tecnico no Field Service

70.08 - Realiza o Encerramento da OS de Instalação
    [TAGS]                                  SCRIPT_B
...
#70.09 - Validar mudanças de estado no FW 
    #[Tags]                                  SCRIPT_B
    # Step MicroServiços             
70.10 - Auditoria de Tarefas
    [TAGS]                                  SCRIPT_B
    Auditoria de Tarefas

70.11 - Realizar Validação de Retorno via SOM
    [TAGS]                                  SCRIPT_B
